...
export class DiagnosticSink {
  ...
  addError(message: string, range: Range) {
    return this.addDiagnostic(
      new Diagnostic(DiagnosticCategory.Error, message, range)
    );
  }

  addWarning(message: string, range: Range) {
    return this.addDiagnostic(
      new Diagnostic(DiagnosticCategory.Warning, message, range)
    );
  }

  addInformation(message: string, range: Range) {
    return this.addDiagnostic(
      new Diagnostic(DiagnosticCategory.Information, message, range)
    );
  }
  ...
  addDiagnostic(diag: Diagnostic) {
    ...
    const key =
      ...
      `${diag.range.end.line}-${diag.range.end.character}:${hashString(
        diag.message
      )}}`;
  ...
  getInformation() {
    return this._diagnosticList.filter(
      (diag) => diag.category === DiagnosticCategory.Information
    );
  }
  ...
  getUnreachableCode() {
    return this._diagnosticList.filter(
      (diag) => diag.category === DiagnosticCategory.UnreachableCode
    );
  }

  getDeprecated() {
    return this._diagnosticList.filter(
      (diag) => diag.category === DiagnosticCategory.Deprecated
    );
  }
}
...
export class TextRangeDiagnosticSink extends DiagnosticSink {
  ...
  addDiagnosticWithTextRange(
    level: any,
    // level: DiagnosticLevel,
    message: string,
    range: TextRange
  ) {
    const positionRange = convertOffsetsToRange(
      range.start,
