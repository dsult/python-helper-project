...
export function computeCompletionSimilarity(
  ...
  while (symbolSubstrLength > 0) {
    const editDistance = leven(
      symbolLower.substr(0, symbolSubstrLength),
      typedLower
    );
  ...
  const similarity =
    (typedValue.length - smallestEditDistance) / typedValue.length;
...
export function isPatternInSymbol(
  typedValue: string,
  symbolName: string
): boolean {
  const typedLower = typedValue.toLocaleLowerCase();
