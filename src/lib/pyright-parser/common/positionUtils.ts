...
export function convertOffsetToPosition(
  ...
  const itemIndex =
    offset >= lines.end ? lines.count - 1 : lines.getItemContaining(offset);
...
export function convertTextRangeToRange(
  range: TextRange,
  lines: TextRangeCollection<TextRange>
): Range {
...
export function getLineEndPosition(
