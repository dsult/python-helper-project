...
export namespace Position {
  export function is(value: any): value is Position {
    ...
    return (
      candidate && candidate.line !== void 0 && candidate.character !== void 0
    );
  }
...
export function doesRangeContain(
  ...
  if (Position.is(positionOrRange)) {
    return (
      comparePositions(range.start, positionOrRange) <= 0 &&
      comparePositions(range.end, positionOrRange) >= 0
    );
  }

  return (
    doesRangeContain(range, positionOrRange.start) &&
    doesRangeContain(range, positionOrRange.end)
  );
}
...
export function extendRange(
  range: Range,
  extension: Range | Range[] | undefined
) {
  if (extension) {
    if (Array.isArray(extension)) {
      extension.forEach((r) => {
