...
const _operatorInfo: { [key: number]: OperatorFlags } = {
  ...
  [OperatorType.GreaterThanOrEqual]:
    OperatorFlags.Binary | OperatorFlags.Comparison,
  ...
  [OperatorType.LessOrGreaterThan]:
    OperatorFlags.Binary | OperatorFlags.Comparison | OperatorFlags.Deprecated,
...
export class Tokenizer {
  ...
  tokenize(
    ...
    if (this._lineRanges.length > 0) {
      ...
      const lastCharOfLastLine = text.charCodeAt(
        lastLine.start + lastLine.length - 1
      );
      if (
        lastCharOfLastLine === Char.CarriageReturn ||
        lastCharOfLastLine === Char.LineFeed
      ) {
  ...
  static isOperatorAssignment(operatorType?: OperatorType): boolean {
    if (
      ...
      _operatorInfo[operatorType] === undefined
    ) {
      return false;
    }
    ...
    if (stringPrefixLength >= 0) {
      ...
      if (stringPrefixLength > 0) {
        stringPrefix = this._cs
          .getText()
          .slice(this._cs.position, this._cs.position + stringPrefixLength);
        // Indeed a string
    ...
    if (this._indentAmounts.length === 0) {
      ...
      if (prevTabInfo.tab8Spaces < tab8Spaces) {
        ...
        this._indentSpacesTotal +=
          tab8Spaces -
          this._indentAmounts[this._indentAmounts.length - 1].tab8Spaces;
  ...
  private _tryNumber(): boolean {
    ...
    if (this._cs.currentChar === Char._0) {
      ...
      if (radix > 0) {
        ...
        let intValue: number | bigint = parseInt(
          simpleIntText.slice(leadingChars),
          radix
        );
  ...
  private _tryOperator(): boolean {
    ...
    switch (this._cs.currentChar) {
      ...
      case Char.Hyphen:
        ...
        length = nextChar === Char.Equal ? 2 : 1;
        operatorType =
          length === 2 ? OperatorType.SubtractEqual : OperatorType.Subtract;
        break;
      ...
      case Char.Asterisk:
        if (nextChar === Char.Asterisk) {
          ...
          operatorType =
            length === 3 ? OperatorType.PowerEqual : OperatorType.Power;
        } else {
          length = nextChar === Char.Equal ? 2 : 1;
          operatorType =
      ...
      case Char.Slash:
        if (nextChar === Char.Slash) {
          ...
          length = nextChar === Char.Equal ? 2 : 1;
          operatorType =
            length === 2 ? OperatorType.DivideEqual : OperatorType.Divide;
        }
        break;
  ...
  private _handleIPythonMagics(type: CommentType): void {
    ...
    do {
      ...
      if (
        type === CommentType.IPythonMagic ||
        type === CommentType.IPythonShellEscape
      ) {
    ...
    const comment = Comment.create(
      start,
      length,
      this._cs.getText().slice(start, start + length),
      type
    );
  ...
  private _handleComment(): void {
    ...
    const typeIgnoreRegexMatch = comment.value.match(
      /((^|#)\s*)type:\s*ignore(\s*\[([\s*\w-,]*)\]|\s|$)/
    );
    if (typeIgnoreRegexMatch) {
      ...
      if (
        this._tokens.findIndex(
          (t) =>
            t.type !== TokenType.NewLine && t && t.type !== TokenType.Indent
        ) < 0
      ) {
    ...
    const pyrightIgnoreRegexMatch = comment.value.match(
      /((^|#)\s*)pyright:\s*ignore(\s*\[([\s*\w-,]*)\]|\s|$)/
    );
    if (pyrightIgnoreRegexMatch) {
      ...
      const textRange: TextRange = {
        ...
        length:
          pyrightIgnoreRegexMatch[0].length - pyrightIgnoreRegexMatch[1].length,
      };
      const ignoreComment: IgnoreComment = {
        range: textRange,
        rulesList: this._getIgnoreCommentRulesList(
  ...
  private _getIgnoreCommentRulesList(
    start: number,
    match: RegExpMatchArray
  ): IgnoreCommentRule[] | undefined {
  ...
  private _handleFStringMiddle(): void {
    ...
    const inFormatSpecifier =
      !!this._activeFString!.activeReplacementField?.inFormatSpecifier;
    const start = this._cs.position;
    const flags = activeFString.startToken.flags;
    const stringLiteralInfo = this._skipToEndOfStringLiteral(
      flags,
      inFormatSpecifier
    );
    const end = this._cs.position;
    ...
    const isUnterminated =
      (stringLiteralInfo.flags & StringTokenFlags.Unterminated) !== 0;
    const sawReplacementFieldStart =
      (stringLiteralInfo.flags & StringTokenFlags.ReplacementFieldStart) !== 0;
    const sawReplacementFieldEnd =
      (stringLiteralInfo.flags & StringTokenFlags.ReplacementFieldEnd) !== 0;
    const sawEndQuote =
      !isUnterminated && !sawReplacementFieldStart && !sawReplacementFieldEnd;
  ...
  private _skipToEndOfStringLiteral(
    flags: StringTokenFlags,
    inFormatSpecifier = false
  ): StringScannerOutput {
    const quoteChar =
    ...
    while (true) {
      ...
      if (this._cs.currentChar === Char.Backslash) {
        ...
        if (
          ...
          const isEscapedBackslash =
            this._cs.getCurrentChar() === Char.Backslash;
        ...
        isInNamedUnicodeEscape &&
        this._cs.currentChar === Char.CloseBrace
      ) {
  ...
  private _skipDecimalNumber(allowSign: boolean): void {
    if (
      allowSign &&
      (this._cs.currentChar === Char.Hyphen ||
        this._cs.currentChar === Char.Plus)
    ) {
      // Optional sign
      this._cs.moveNext();
    }
