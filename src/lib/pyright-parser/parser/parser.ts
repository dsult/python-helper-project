...
export class Parser {
  ...
  parseSourceFile(
    ...
    const moduleNode = ModuleNode.create({
      start: 0,
  ...
  private _parseTypeAliasStatement(): TypeAliasNode {
    ...
    if (
      ...
      this._getLanguageVersion().isLessThan(pythonVersion3_12)
    ) {
  ...
  private _parseTypeParameterList(): TypeParameterListNode {
      ...
      if (firstToken.type === TokenType.CloseBracket) {
        if (typeVariableNodes.length === 0) {
          this._addSyntaxError(
            LocMessage.typeParametersMissing(),
            this._peekToken()
          );
        }
        break;
    ...
    return TypeParameterListNode.create(
      openBracketToken,
      closingToken,
      typeVariableNodes
    );
  ...
  private _parseTypeParameter(): TypeParameterNode | undefined {
    ...
    if (!nameToken) {
      this._addSyntaxError(
        LocMessage.expectedTypeParameterName(),
        this._peekToken()
      );
      return undefined;
    }
    ...
    if (this._consumeTokenIfType(TokenType.Colon)) {
      ...
      if (typeParamCategory !== TypeParameterCategory.TypeVar) {
        this._addSyntaxError(
          LocMessage.typeParameterBoundNotAllowed(),
          boundExpression
        );
      }
    }
    ...
    if (this._consumeTokenIfOperator(OperatorType.Assign)) {
      defaultExpression = this._parseExpression(
        /* allowUnpack */ typeParamCategory ===
          TypeParameterCategory.TypeVarTuple
      );

      if (
        !this._parseOptions.isStubFile &&
        this._getLanguageVersion().isLessThan(pythonVersion3_13)
      ) {
        this._addSyntaxError(
  ...
  private _parseMatchStatement(): MatchNode | undefined {
    ...
    this._suppressErrors(() => {
      ...
      smellsLikeMatchStatement =
        expression.nodeType !== ParseNodeType.Error &&
    ...
    if (!this._consumeTokenIfType(TokenType.Colon)) {
      ...
      if (!this._consumeTokenIfType(TokenType.NewLine)) {
        ...
        if (!this._consumeTokenIfType(TokenType.Indent)) {
          this._addSyntaxError(
            LocMessage.expectedIndentedBlock(),
            this._peekToken()
          );
        } else {
        ...
        while (true) {
          ...
          if (!caseStatement) {
            // Perform basic error recovery to get to the next line.
            if (
              this._consumeTokensUntilType([TokenType.NewLine, TokenType.Colon])
            ) {
              this._getNextToken();
            }
          } else {
    ...
    for (let i = 0; i < matchNode.cases.length - 1; i++) {
      ...
      if (!caseNode.guardExpression && caseNode.isIrrefutable) {
        this._addSyntaxError(
          LocMessage.casePatternIsIrrefutable(),
          caseNode.pattern
        );
      }
    }
  ...
  private _parseCaseStatement(): CaseNode | undefined {
    ...
    if (patternList.parseError) {
      ...
      if (pattern.nodeType === ParseNodeType.PatternCapture && pattern.isStar) {
        casePattern = PatternSequenceNode.create(
          patternList.list[0],
          patternList.list
        );
      } else {
  ...
  private _isPatternIrrefutable(node: PatternAtomNode): boolean {
    ...
    if (node.nodeType === ParseNodeType.PatternAs) {
      return node.orPatterns.some((pattern) =>
  ...
  private _reportDuplicatePatternCaptureTargets(
    ...
    const reportTargetIfDuplicate = (nameNode: NameNode) => {
      if (
        globalNameMap.has(nameNode.value) ||
        localNameMap.has(nameNode.value)
      ) {
        this._addSyntaxError(
  ...
  private _parsePatternSequence() {
    const patternList = this._parseExpressionListGeneric(() =>
    ...
    if (starEntries.length > 1) {
      this._addSyntaxError(
        LocMessage.duplicateStarPattern(),
        starEntries[1].orPatterns[0]
      );
    }
  ...
  private _parsePatternAs(): PatternAsNode {
    ...
    if (orPatterns.length > 1) {
      ...
      orPatterns.forEach((patternAtom) => {
        if (
          ...
          this._addSyntaxError(
            LocMessage.starPatternInOrPattern(),
            patternAtom
          );
        }
      });
    ...
    if (this._consumeTokenIfKeyword(KeywordType.As)) {
      ...
      if (nameToken) {
        ...
        this._addSyntaxError(
          LocMessage.expectedNameAfterAs(),
          this._peekToken()
        );
      }
    }
    ...
    orPatterns.forEach((orPattern) => {
      ...
      if (localNameSet.size < fullNameSet.size) {
        const missingNames = Array.from(fullNameSet.keys()).filter(
        ...
        this._addSyntaxError(
          LocMessage.orPatternMissingName() + diag.getString(),
          orPattern
        );
      }
    });
  ...
  private _parsePatternAtom(): PatternAtomNode {
    ...
    if (nextOperator === OperatorType.Multiply) {
      ...
      if (!identifierToken) {
        ...
        return PatternCaptureNode.create(
          NameNode.create(identifierToken),
          starToken
        );
      }
    }
    ...
    if (
      ...
      nextToken.type === TokenType.OpenBracket
    ) {
      ...
      if (patternList.parseError) {
        ...
        if (
          pattern.nodeType === ParseNodeType.PatternCapture &&
          pattern.isStar
        ) {
          casePattern = PatternSequenceNode.create(
            startToken,
  ...
  private _parseClassPatternArgument(): PatternClassArgumentNode {
    ...
    if (
      (firstToken.type === TokenType.Identifier ||
        firstToken.type === TokenType.Keyword) &&
  ...
  private _parsePatternLiteralNumber(): PatternLiteralNode {
    ...
    if (expression.nodeType === ParseNodeType.BinaryOperation) {
      if (
        expression.operator === OperatorType.Subtract ||
        expression.operator === OperatorType.Add
      ) {
    ...
    if (realValue) {
      if (
        realValue.nodeType === ParseNodeType.UnaryOperation &&
        realValue.operator === OperatorType.Subtract
      ) {
        realValue = realValue.expression;
      }

      if (
        realValue.nodeType !== ParseNodeType.Number ||
        (imagValue !== undefined && realValue.isImaginary)
      ) {
        this._addSyntaxError(
          LocMessage.expectedComplexNumberLiteral(),
          expression
        );
        imagValue = undefined;
      }
    ...
    if (imagValue) {
      if (
        imagValue.nodeType === ParseNodeType.UnaryOperation &&
        imagValue.operator === OperatorType.Subtract
      ) {
        imagValue = imagValue.expression;
      }

      if (
        imagValue.nodeType !== ParseNodeType.Number ||
        !imagValue.isImaginary
      ) {
        this._addSyntaxError(
          LocMessage.expectedComplexNumberLiteral(),
          expression
  ...
  private _parsePatternMapping(
    firstToken: Token
  ): PatternMappingNode | ErrorNode {
    const itemList = this._parseExpressionListGeneric(() =>
    ...
    if (itemList.list.length > 0) {
      ...
      if (starStarEntries.length > 1) {
        this._addSyntaxError(
          LocMessage.duplicateStarStarPattern(),
          starStarEntries[1]
        );
      }
    ...
    if (patternLiteral) {
      ...
      if (patternCaptureOrValue) {
        if (patternCaptureOrValue.nodeType === ParseNodeType.PatternValue) {
          ...
          this._addSyntaxError(
            LocMessage.expectedPatternValue(),
  ...
  private _parsePatternCaptureOrValue():
    | PatternCaptureNode
    | PatternValueNode
    | ErrorNode
    | undefined {
      ...
      while (true) {
        ...
        if (identifierToken) {
          ...
          nameOrMember = nameOrMember
            ? MemberAccessNode.create(nameOrMember, nameNode)
            : nameNode;
        } else {
          this._addSyntaxError(
            LocMessage.expectedIdentifier(),
    ...
    if (this._consumeTokenIfType(TokenType.NewLine)) {
      ...
      while (true) {
        ...
        if (nextToken.type === TokenType.Indent) {
          ...
          if (
            !bodyIndentToken ||
            dedentToken.indentAmount < bodyIndentToken.indentAmount
          ) {
            break;
          } else if (
            dedentToken.indentAmount === bodyIndentToken.indentAmount
  ...
  private _parseForStatement(asyncToken?: KeywordToken): ForNode {
    ...
    if (!this._consumeTokenIfKeyword(KeywordType.In)) {
      ...
      if (
        this._getLanguageVersion().isLessThan(pythonVersion3_9) &&
        !this._parseOptions.isStubFile
      ) {
        if (
          seqExpr.nodeType === ParseNodeType.Tuple &&
          !seqExpr.enclosedInParens
        ) {
          let sawStar = false;
  ...
  private _tryParseListComprehension(
    ...
    if (target.nodeType === ParseNodeType.Unpack) {
      ...
      this._addSyntaxError(
        LocMessage.dictExpandIllegalInComprehension(),
        target
      );
    }
  ...
  private _parseTryStatement(): TryNode {
    ...
    while (true) {
      ...
      if (this._consumeTokenIfOperator(OperatorType.Multiply)) {
        if (
          ...
          this._addSyntaxError(
            LocMessage.exceptionGroupIncompatible(),
            possibleStarToken
          );
        }
        isExceptGroup = true;
      ...
      if (this._peekTokenType() !== TokenType.Colon) {
        ...
        if (this._consumeTokenIfKeyword(KeywordType.As)) {
          ...
          if (this._consumeTokenIfType(TokenType.Comma)) {
            this._addSyntaxError(
              LocMessage.expectedAsAfterException(),
              peekToken
            );
      ...
      const exceptNode = ExceptNode.create(
        exceptToken,
        exceptSuite,
        isExceptGroup
      );
    ...
    if (tryNode.exceptClauses.length > 0) {
      extendRange(
        tryNode,
        tryNode.exceptClauses[tryNode.exceptClauses.length - 1]
      );
  ...
  private _parseFunctionDef(
    ...
    if (possibleOpenBracket.type === TokenType.OpenBracket) {
      ...
      if (
        ...
        this._addSyntaxError(
          LocMessage.functionTypeParametersIllegal(),
          typeParameters
        );
      }
    }
  ...
  private _parseVarArgsList(
    ...
    while (true) {
      ...
      if (param.name) {
        ...
        if (paramMap.has(name)) {
          this._addSyntaxError(
            LocMessage.duplicateParam().format({ name }),
            param.name
          );
        } else {
      ...
      if (param.category === ParameterCategory.Simple) {
        if (!param.name) {
          if (sawPositionOnlySeparator) {
            ...
            this._addSyntaxError(
              LocMessage.positionOnlyAfterKeywordOnly(),
              param
            );
          } else if (sawArgs) {
    ...
    if (paramList.length > 0) {
      ...
      if (
        lastParam.category === ParameterCategory.ArgsList &&
        !lastParam.name
      ) {
  ...
  private _parseParameter(allowAnnotations: boolean): ParameterNode {
    ...
    if (this._consumeTokenIfOperator(OperatorType.Multiply)) {
      ...
      if (
        this._getLanguageVersion().isLessThan(pythonVersion3_8) &&
        !this._parseOptions.isStubFile
      ) {
    ...
    if (!paramName) {
      if (starCount === 1) {
        const paramNode = ParameterNode.create(
          firstToken,
      ...
      if (this._peekTokenType() === TokenType.OpenParenthesis) {
        ...
        this._addSyntaxError(
          LocMessage.sublistParamsIncompatible(),
          sublistStart
        );
      } else {
    ...
    if (this._consumeTokenIfOperator(OperatorType.Assign)) {
      ...
      if (starCount > 0) {
        this._addSyntaxError(
          LocMessage.defaultValueNotAllowed(),
  ...
  private _parseWithStatement(asyncToken?: KeywordToken): WithNode {
      ...
      this._suppressErrors(() => {
        ...
        if (
          ...
          isParenthesizedWithItemList =
            withItemList.length !== 1 || withItemList[0].target !== undefined;
        }
    ...
    if (isParenthesizedWithItemList) {
      ...
      if (this._getLanguageVersion().isLessThan(pythonVersion3_9)) {
        this._addSyntaxError(
          LocMessage.parenthesizedContextManagerIllegal(),
          possibleParen
        );
      }
    }
  ...
  private _parseDecorated(): StatementNode | undefined {
    ...
    if (nextToken.type === TokenType.Keyword) {
      if (nextToken.keywordType === KeywordType.Async) {
        ...
        if (this._peekKeywordType() !== KeywordType.Def) {
          this._addSyntaxError(
            LocMessage.expectedFunctionAfterAsync(),
            this._peekToken()
          );
        } else {
    ...
    this._addSyntaxError(
      LocMessage.expectedAfterDecorator(),
      this._peekToken()
    );
  ...
  private _parseDecorator(): DecoratorNode {
    ...
    if (!this._consumeTokenIfType(TokenType.NewLine)) {
      this._addSyntaxError(
        LocMessage.expectedDecoratorNewline(),
        this._peekToken()
      );
      this._consumeTokensUntilType([TokenType.NewLine]);
    }
  ...
  private _parseClassDef(decorators?: DecoratorNode[]): ClassNode {
    ...
    if (possibleOpenBracket.type === TokenType.OpenBracket) {
      ...
      if (
        ...
        this._addSyntaxError(
          LocMessage.classTypeParametersIllegal(),
          typeParameters
        );
      }
    }
  ...
  private _parseFromStatement(): ImportFromNode {
      ...
      if (this._consumeTokenIfOperator(OperatorType.Multiply)) {
        ...
        while (true) {
          ...
          if (this._consumeTokenIfKeyword(KeywordType.As)) {
            ...
            if (!aliasName) {
              this._addSyntaxError(
                LocMessage.expectedImportAlias(),
                this._peekToken()
              );
            } else {
        ...
        if (importFromNode.imports.length === 0) {
          this._addSyntaxError(
            LocMessage.expectedImportSymbols(),
            this._peekToken()
          );
        }
        ...
        if (inParen) {
          ...
          this._addSyntaxError(
            LocMessage.trailingCommaInFromImport(),
            trailingCommaToken
          );
        }
      }
    ...
    this._importedModules.push({
      ...
      importedSymbols: new Set<string>(
        importFromNode.imports.map((imp) => imp.name.value)
      ),
    });
    ...
    if (importFromNode.module.nameParts.length === 1) {
      ...
      if (
        firstNamePartValue === "typing" ||
        firstNamePartValue === "typing_extensions"
      ) {
        isTypingImport = true;
      }
    }
  ...
  private _parseImportStatement(): ImportNode {
    ...
    while (true) {
      ...
      if (importAsNode.module.leadingDots > 0) {
        this._addSyntaxError(
          LocMessage.relativeImportNotAllowed(),
          importAsNode.module
        );
      }
      ...
      if (modName.nameParts.length === 1) {
        ...
        if (
          ...
          this._typingImportAliases.push(
            importAsNode.alias?.value || firstNamePartValue
          );
        }
      }
  ...
  private _parseDottedModuleName(allowJustDots = false): ModuleNameNode {
    ...
    while (true) {
      const token =
        this._getTokenIfType(TokenType.Ellipsis) ??
        this._getTokenIfType(TokenType.Dot);
      if (token) {
    ...
    while (true) {
      ...
      if (!identifier) {
        if (
          ...
          moduleNameNode.nameParts.length > 0
        ) {
          this._addSyntaxError(
            LocMessage.expectedModuleName(),
            this._peekToken()
          );
  ...
  private _parseGlobalStatement(): GlobalNode {
    ...
    if (globalNode.nameList.length > 0) {
      ...
      extendRange(
        globalNode,
        globalNode.nameList[globalNode.nameList.length - 1]
      );
    }
    return globalNode;
  }
  ...
  private _parseNonlocalStatement(): NonlocalNode {
    ...
    if (nonlocalNode.nameList.length > 0) {
      ...
      extendRange(
        nonlocalNode,
        nonlocalNode.nameList[nonlocalNode.nameList.length - 1]
      );
    }
    return nonlocalNode;
  }
  ...
  private _parseSimpleStatement(): StatementListNode {
    ...
    while (true) {
      ...
      if (this._peekTokenType() === TokenType.Invalid) {
        ...
        const text = this._fileContents!.substr(
          invalidToken.start,
          invalidToken.length
        );
        ...
        this._addSyntaxError(
          LocMessage.invalidTokenChars().format({
    ...
    if (!this._consumeTokenIfType(TokenType.NewLine)) {
      this._addSyntaxError(
        LocMessage.expectedNewlineOrSemicolon(),
        this._peekToken()
      );
    }
  ...
  private _makeExpressionOrTuple(
    ...
    if (exprListResult.list.length === 1 && !exprListResult.trailingComma) {
      if (exprListResult.list[0].nodeType === ParseNodeType.Unpack) {
        this._addSyntaxError(
          LocMessage.unpackOperatorNotAllowed(),
  ...
  private _parseExpression(allowUnpack: boolean): ExpressionNode {
    ...
    if (allowUnpack && this._consumeTokenIfOperator(OperatorType.Multiply)) {
      return UnpackNode.create(
        startToken,
        this._parseExpression(/* allowUnpack */ false)
      );
    }
  ...
  private _parseTestOrStarExpression(
    allowAssignmentExpression: boolean
  ): ExpressionNode {
    ...
    while (true) {
      ...
      if (Tokenizer.isOperatorComparison(this._peekOperatorType())) {
        ...
        if (comparisonOperator === OperatorType.LessOrGreaterThan) {
          this._addSyntaxError(
            LocMessage.operatorLessOrGreaterDeprecated(),
            peekToken
          );
      ...
      if (this._consumeTokenIfType(TokenType.OpenParenthesis)) {
        ...
        if (argListResult.args.length > 1 || argListResult.trailingComma) {
          argListResult.args.forEach((arg) => {
            if (
              arg.valueExpression.nodeType === ParseNodeType.ListComprehension
            ) {
              if (!arg.valueExpression.isParenthesized) {
                this._addSyntaxError(
                  LocMessage.generatorNotParenthesized(),
                  arg.valueExpression
                );
              }
            }
        ...
        if (
          atomExpression.maxChildDepth !== undefined &&
          atomExpression.maxChildDepth >= maxChildNodeDepth
        ) {
          ...
          this._addSyntaxError(
            LocMessage.maxParseDepthExceeded(),
            atomExpression
          );
        }
  ...
  private _parseSubscriptList(): SubscriptListResult {
    ...
    while (true) {
      ...
      if (
        firstToken.type !== TokenType.Colon &&
      ...
      if (argType === ArgumentCategory.Simple) {
        if (this._consumeTokenIfOperator(OperatorType.Assign)) {
          ...
          if (
            !this._parseOptions.isStubFile &&
            this._getLanguageVersion().isLessThan(pythonVersion3_10)
          ) {
            this._addSyntaxError(
              LocMessage.assignmentExprInSubscript(),
              valueExpr
            );
          }
        }
      ...
      if (argNode.name) {
        ...
        sawKeywordArg &&
        argNode.argumentCategory === ArgumentCategory.Simple
      ) {
      ...
      if (argNode.name) {
        this._addSyntaxError(
          LocMessage.keywordSubscriptIllegal(),
          argNode.name
        );
      }
      ...
      if (argType !== ArgumentCategory.Simple) {
        ...
        if (argType === ArgumentCategory.UnpackedDictionary) {
          this._addSyntaxError(
            LocMessage.unpackedDictSubscriptIllegal(),
            argNode
          );
        }
      }
  ...
  private _parsePossibleSlice(): ExpressionNode {
    ...
    while (true) {
      const nextTokenType = this._peekTokenType();
      if (
        nextTokenType === TokenType.CloseBracket ||
      ...
      if (nextTokenType !== TokenType.Colon) {
        ...
        const allowAssignmentExpression =
          this._parseOptions.isStubFile ||
          this._getLanguageVersion().isGreaterOrEqualTo(pythonVersion3_10);
  ...
  private _parseArgument(): ArgumentNode {
    ...
    if (argType === ArgumentCategory.Simple) {
      if (this._consumeTokenIfOperator(OperatorType.Assign)) {
        ...
        const listComp = this._tryParseListComprehension(
          valueExpr,
          /* isGenerator */ true
        );
        if (listComp) {
  ...
  private _parseAtom(): ExpressionNode {
    ...
    if (
      ...
      nextToken.type === TokenType.FStringStart
    ) {
  ...
  private _handleExpressionParseError(
    ...
    const initialRange: TextRange = stopTokens.some((k) => nextToken.type === k)
      ? targetToken ??
        childNode ??
        TextRange.create(nextToken.start, /* length */ 0)
      : nextToken;
  ...
  private _parseLambdaExpression(allowConditional = true): LambdaNode {
    ...
    if (allowConditional) {
      ...
      testExpr =
        this._tryParseLambdaExpression(/* allowConditional */ false) ||
        this._parseOrTest();
    }
  ...
  private _tryParseLambdaExpression(
  ...
  private _parseTestListWithComprehension(
    isGenerator: boolean
  ): ListResult<ExpressionNode> {
    ...
    while (true) {
      ...
      if (keyExpression && valueExpression) {
        ...
        if (isSet) {
          ...
          const listEntryNode =
            DictionaryExpandEntryNode.create(doubleStarExpression);
          ...
          if (listComp) {
            ...
            if (!isFirstEntry) {
              this._addSyntaxError(
                LocMessage.comprehensionInDict(),
  ...
  private _parseExpressionStatement(): ExpressionNode {
    ...
    if (this._consumeTokenIfType(TokenType.Colon)) {
      ...
      if (
        !this._parseOptions.isStubFile &&
        this._getLanguageVersion().isLessThan(pythonVersion3_6)
      ) {
      ...
      const isTypeAliasDeclaration = this._isTypingAnnotation(
        annotationExpr,
        "TypeAlias"
      );
  ...
  private _parseFunctionTypeAnnotation(): FunctionAnnotationNode | undefined {
    ...
    while (true) {
      ...
      this._consumeTokenIfOperator(OperatorType.Multiply) ||
        this._consumeTokenIfOperator(OperatorType.Power);
    ...
    if (
      ...
      paramAnnotations[0].nodeType === ParseNodeType.Ellipsis
    ) {
      paramAnnotations = [];
  ...
  private _getTypeAnnotationCommentText(): StringToken | undefined {
    ...
    const curToken = this._tokenizerOutput!.tokens.getItemAt(
      this._tokenIndex - 1
    );
    ...
    const interTokenContents = this._fileContents!.slice(
  ...
  private _parseVariableTypeAnnotationComment(): ExpressionNode | undefined {
    ...
    assert(
      parseResults.parseTree.nodeType !== ParseNodeType.FunctionAnnotation
    );
  ...
  private _parseFStringReplacementField(
    ...
    if (
      ...
      if (this._parseOptions.pythonVersion.isLessThan(pythonVersion3_8)) {
        this._addSyntaxError(
          LocMessage.formatStringDebuggingIllegal(),
          nextToken
        );
      }
    ...
    if (nextToken.type === TokenType.ExclamationMark) {
      ...
      if (nextToken.type !== TokenType.Identifier) {
        this._addSyntaxError(
          LocMessage.formatStringExpectedConversion(),
          nextToken
        );
      } else {
        this._getNextToken();
  ...
  private _parseFStringFormatString(
    ...
    while (true) {
      ...
      if (
        ...
        nextToken.type === TokenType.FStringEnd
      ) {
        break;
      }
      ...
      if (nextToken.type === TokenType.OpenCurlyBrace) {
        ...
        if (nestingDepth === 2) {
          this._addSyntaxError(
            LocMessage.formatStringNestedFormatSpecifier(),
            nextToken
          );
        }
  ...
  private _createBinaryOperationNode(
    ...
    // or type evaluator.
    if (
      leftExpression.maxChildDepth !== undefined &&
      leftExpression.maxChildDepth >= maxChildNodeDepth
    ) {
      leftExpression = ErrorNode.create(
        leftExpression,
        ErrorExpressionCategory.MaxDepthExceeded
      );
    ...
    if (
      rightExpression.maxChildDepth !== undefined &&
      rightExpression.maxChildDepth >= maxChildNodeDepth
    ) {
      rightExpression = ErrorNode.create(
        rightExpression,
        ErrorExpressionCategory.MaxDepthExceeded
      );
  ...
  private _createUnaryOperationNode(
    ...
    // or type evaluator.
    if (
      expression.maxChildDepth !== undefined &&
      expression.maxChildDepth >= maxChildNodeDepth
    ) {
      expression = ErrorNode.create(
        expression,
        ErrorExpressionCategory.MaxDepthExceeded
      );
  ...
  private _parseStringList(): StringListNode {
    ...
    while (true) {
      ...
      if (nextToken.type === TokenType.String) {
        stringList.push(
          this._makeStringNode(this._getNextToken() as StringToken)
        );
      } else if (nextToken.type === TokenType.FStringStart) {
        stringList.push(
          this._parseFormatString(this._getNextToken() as FStringStartToken)
        );
      } else {
        break;
      }
    ...
    if (this._isParsingTypeAnnotation) {
      ...
      if (stringNode.strings.length > 1) {
        ...
        stringNode.strings[0].nodeType === ParseNodeType.FormatString
      ) {
        if (this._isParsingQuotedText) {
        ...
        if (
          ...
          if (this._isParsingQuotedText) {
            this._addSyntaxError(
              LocMessage.annotationStringEscape(),
              stringNode
            );
          }
        } else {
          ...
          const parseResults = parser.parseTextExpression(
            ...
            this._parseOptions,
            ParseTextMode.VariableAnnotation,
            (stringNode.strings[0].token.flags &
          ...
          if (
            ...
            if (parseResults.parseTree) {
              assert(
                parseResults.parseTree.nodeType !==
                  ParseNodeType.FunctionAnnotation
              );
  ...
  private _getTokenIfIdentifier(): IdentifierToken | undefined {
    ...
    if (nextToken.type === TokenType.Keyword) {
      ...
      if (softKeywords.find((type) => type === keywordType)) {
        const keywordText = this._fileContents!.substr(
          nextToken.start,
          nextToken.length
        );
        this._getNextToken();
