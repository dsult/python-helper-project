...
import {
  TextRangeCollection,
  getIndexContaining,
} from "../common/textRangeCollection";
import {
  ArgumentCategory,
  ArgumentNode,
...
export function findNodeByOffset(
  ...
  for (const child of children) {
    ...
    if (containingChild) {
      ...
      if (
        node.nodeType === ParseNodeType.AugmentedAssignment &&
        containingChild === node.leftExpression
      ) {
...
export function printExpression(
  ...
  switch (node.nodeType) {
    ...
    case ParseNodeType.UnaryOperation: {
      const exprStr =
        printOperator(node.operator) + printExpression(node.expression, flags);
    ...
    case ParseNodeType.StringList: {
      if (
        flags & PrintExpressionFlags.ForwardDeclarations &&
        node.typeAnnotation
      ) {
    ...
    case ParseNodeType.ListComprehension: {
      ...
      if (isExpressionNode(node.expression)) {
        ...
        node.expression.nodeType === ParseNodeType.DictionaryKeyEntry
      ) {
        const keyStr = printExpression(node.expression.keyExpression, flags);
        const valueStr = printExpression(
...
export function getDecoratorForName(node: NameNode): DecoratorNode | undefined {
  if (
    node.parent?.nodeType === ParseNodeType.Decorator &&
...
export function getEnclosingClassOrFunction(
  node: ParseNode
        ...
        if (getScope(curNode) !== undefined) {
          ...
          const isFirstIterableExpr =
            ...
            curNode.forIfNodes[0].nodeType ===
              ParseNodeType.ListComprehensionFor &&
...
export function getTypeAnnotationNode(
  node: ParseNode
...
export function getArgumentsByRuntimeOrder(node: CallNode) {
  const positionalArgs = node.arguments.filter(
    (arg) =>
      !arg.name && arg.argumentCategory !== ArgumentCategory.UnpackedDictionary
  );
...
export function isFinalAllowedForAssignmentTarget(
...
export function isClassVarAllowedForAssignmentTarget(
  targetNode: ExpressionNode
): boolean {
...
export function isRequiredAllowedForAssignmentTarget(
  targetNode: ExpressionNode
): boolean {
  const classNode = getEnclosingClass(targetNode, /* stopAtFunction */ true);
  if (!classNode) {
...
export function getParentNodeOfType<T extends ParseNode>(
  node: ParseNode,
  containerType: ParseNodeType
): T | undefined {
  let curNode: ParseNode | undefined = node;
...
export function getParentAnnotationNode(
  node: ExpressionNode
  ...
  while (curNode) {
    ...
    if (curNode.nodeType === ParseNodeType.Parameter) {
      if (
        prevNode === curNode.typeAnnotation ||
        prevNode === curNode.typeAnnotationComment
      ) {
        return prevNode;
      }
      return undefined;
    ...
    if (curNode.nodeType === ParseNodeType.FunctionAnnotation) {
      if (
        prevNode === curNode.returnTypeAnnotation ||
        curNode.paramTypeAnnotations.some((p) => p === prevNode)
      ) {
...
export function isNodeContainedWithinNodeType(
  node: ParseNode,
  containerType: ParseNodeType
): boolean {
  ...
  if (reference.nodeType === ParseNodeType.Name) {
    ...
    return (
      isMatchingExpression(
        reference.leftExpression,
        expression.leftExpression
      ) && reference.memberName.value === expression.memberName.value
    );
  } else if (
    reference.nodeType === ParseNodeType.Index &&
    expression.nodeType === ParseNodeType.Index
  ) {
    if (
      !isMatchingExpression(reference.baseExpression, expression.baseExpression)
    ) {
      return false;
    }
    ...
    if (expr.nodeType === ParseNodeType.StringList) {
      ...
      if (
        ...
        return (
          referenceStringListNode.strings[0].value ===
          subscriptNode.strings[0].value
        );
      }
    }
...
export function isPartialMatchingExpression(
  reference: ExpressionNode,
  expression: ExpressionNode
): boolean {
...
export function isWithinTypeAnnotation(
  ...
  while (curNode) {
    ...
    if (
      curNode.nodeType === ParseNodeType.Function &&
      prevNode === curNode.functionAnnotationComment
    ) {
...
export function isWithinTryBlock(
  node: ParseNode,
  treatWithAsTryBlock = false
): boolean {
  let curNode: ParseNode | undefined = node;
  let prevNode: ParseNode | undefined;
...
export function isAssignmentToDefaultsFollowingNamedTuple(
  callNode: ParseNode
): boolean {
  if (
  ...
  let statementIndex = moduleOrSuite.statements.findIndex(
    (s) => s === statementList
  );
...
export function getEnclosingParameter(
  node: ParseNode
...
export function getCallNodeAndActiveParameterIndex(
  ...
  if (
    ...
    tokenAtEnd?.type === TokenType.CloseParenthesis
  ) {
    return undefined;
  }
  ...
  function isOffsetInsideCallArgs(
    tokens: TextRangeCollection<Token>,
    node: CallNode,
    offset: number
  ) {
    const argumentStart =
      node.leftExpression.length > 0
        ? TextRange.getEnd(node.leftExpression) - 1
    ...
    if (
      ...
      offset < TextRange.getEnd(nextToken)
    ) {
...
export function getTokenAtLeft(
  ...
  const index = getTokenIndexAtLeft(
    tokens,
    position,
    includeWhitespace,
...
export function getTokenOverlapping(
  tokens: TextRangeCollection<Token>,
  position: number
) {
...
export function getIndexOfTokenOverlapping(
  tokens: TextRangeCollection<Token>,
  position: number
) {
  const index = tokens.getItemAtPosition(position);
...
export function findTokenAfter(
...
export function getCommentsAtTokenIndex(
  tokens: TextRangeCollection<Token>,
  index: number
) {
    ...
    switch (curNode.nodeType) {
      ...
      case ParseNodeType.ImportAs: {
        return (
          ...
          (curNode.module.nameParts.length > 0 &&
            prevNode === curNode.module.nameParts[0])
        );
      }
      ...
      case ParseNodeType.ImportFromAs: {
        return (
          prevNode === curNode.alias ||
          (!curNode.alias && prevNode === curNode.name)
        );
      }
...
export function getTypeAnnotationForParameter(
  node: FunctionNode,
  paramIndex: number
): ExpressionNode | undefined {
  ...
  if (
    ...
    node.functionAnnotationComment.isParamListEllipsis
  ) {
    return undefined;
  }
...
export function isImportModuleName(node: ParseNode): boolean {
  return (
    getFirstAncestorOrSelfOfKind(node, ParseNodeType.ModuleName)?.parent
...
type NodeForType<
  NT extends ParseNodeType,
  T extends ParseNode
> = T extends ParseNode & { nodeType: NT } ? T : never;

export function getFirstAncestorOrSelfOfKind<
  NT extends ParseNodeType,
  T extends ParseNode
>(node: ParseNode | undefined, type: NT): NodeForType<NT, T> | undefined {
  return getFirstAncestorOrSelf(node, (n) => n.nodeType === type) as
    | NodeForType<NT, T>
    | undefined;
}
...
export function getDottedNameWithGivenNodeAsLastName(
  node: NameNode
): MemberAccessNode | NameNode {
...
export function getDottedName(
  node: MemberAccessNode | NameNode
): NameNode[] | undefined {
  // ex) [a] or [a].b
  // simple case, [a]
...
export function getFirstNameOfDottedName(
  node: MemberAccessNode | NameNode
): NameNode | undefined {
  // ex) [a] or [a].b
  if (node.nodeType === ParseNodeType.Name) {
  ...
  return (
    ...
    node.parameters.every(
      (param) =>
        ...
        param.typeAnnotationComment === undefined
    )
  );
}
...
export function getVariableDocStringNode(
  node: ExpressionNode
): StringListNode | undefined {
  ...
  if (
    curNode?.nodeType !== ParseNodeType.Assignment &&
    curNode?.nodeType !== ParseNodeType.TypeAlias
  ) {
  ...
  if (
    !suiteOrModule ||
    (suiteOrModule.nodeType !== ParseNodeType.Module &&
      suiteOrModule.nodeType !== ParseNodeType.Suite)
  ) {
    return undefined;
  }

  const assignmentIndex = suiteOrModule.statements.findIndex(
    (node) => node === parentNode
  );
  if (
    assignmentIndex < 0 ||
    assignmentIndex === suiteOrModule.statements.length - 1
  ) {
    return undefined;
  }
  ...
  if (parentNode?.parent?.nodeType === ParseNodeType.Module) {
    ...
    if (
      ...
      getEnclosingClass(func, /* stopAtFunction */ true)
    ) {
      isValidContext = true;
    }
  }
