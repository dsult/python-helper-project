  ...
  switch (declaration.type) {
    ...
    case DeclarationType.Parameter: {
      if (
        ...
        declaration.node.typeAnnotationComment
      ) {
        return true;
      }
      ...
      if (parameterParent?.nodeType === ParseNodeType.Function) {
        if (
          ...
          const paramAnnotations =
            parameterParent.functionAnnotationComment.paramTypeAnnotations;
...
export function areDeclarationsSame(
  ...
  if (
    decl1.type === DeclarationType.Alias &&
    decl2.type === DeclarationType.Alias
  ) {
    if (
...
export function getNameFromDeclaration(declaration: Declaration) {
  switch (declaration.type) {
    ...
    case DeclarationType.Variable:
      return declaration.node.nodeType === ParseNodeType.Name
...
export function getNameNodeForDeclaration(
...
export function isDefinedInFile(decl: Declaration, fileUri: Uri) {
  if (isAliasDeclaration(decl)) {
    ...
    return getFileInfoFromNode(decl.node)?.fileUri === fileUri;
...
export function resolveAliasDeclaration(
  ...
  while (true) {
    ...
    if (curDeclaration.loadSymbolsFromPath) {
      ...
      if (curDeclaration.submoduleFallback) {
        if (curDeclaration.symbolName) {
          ...
          if (
            ...
            const lookupResult = importLookup(
    ...
    const unvisitedDecls = declarations.filter(
    ...
    if (lookupResult?.isInPyTypedPackage) {
      if (!sawPyTypedTransition) {
        ...
        if (!symbol.isPrivatePyTypedImport()) {
          privatePyTypedImported =
            privatePyTypedImported ?? curDeclaration?.moduleName;
        }
      }
    }
