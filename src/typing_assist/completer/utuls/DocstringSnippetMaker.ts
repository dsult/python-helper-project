...
export function getDocstringSnippet(
...
function getNumpyDocstringSnippet(
  parameters: SyntaxNode[],
  currentNode: SyntaxNode
): string {
  let cursorCounter = 1;
  ...
  const isOnlySelfParam = !!(
    parameters.length === 1 && parameters[0].text === "self"
  );

  if (parameters.length > 0 && !isOnlySelfParam) {
    snippet += "Parameters\n----------\n";
      ...
      switch (parameterNode.type) {
        ...
        case "default_parameter":
          if (parameterNode.firstChild && parameterNode.lastChild) {
            snippet +=
              parameterNode.firstChild.text +
              ": Any = " +
              parameterNode.lastChild.text +
              "\n";
            snippet += "\t$" + cursorCounter + "\n";
            cursorCounter++;
          }
  ...
  const isReturnType =
    currentNode.parent?.parent?.parent?.parent?.namedChildren[2].type ===
    "type";
  if (isReturnType) {
    const returnType =
      currentNode.parent?.parent?.parent?.parent?.namedChildren[2].text;
    snippet += "Returns\n-------\n" + returnType + "\n";
  } else {
    snippet += "Returns\n-------\n${" + cursorCounter + ":None}\n";
  }
...
function getGoogleDocstringSnippet(
  parameters: SyntaxNode[],
  currentNode: SyntaxNode
): string {
  let cursorCounter = 1;
    ...
    for (let i = startParamIndex; i < parameters.length; i++) {
      ...
      switch (parameterNode.type) {
        ...
        case "typed_parameter":
          snippet +=
            "\t" +
            parameterNode.firstNamedChild?.text +
            " (" +
...
function getEpytextDocstringSnippet(
  parameters: SyntaxNode[],
  currentNode: SyntaxNode
): string {
  let cursorCounter = 1;
  ...
  const startParamIndex =
    parameters.length > 0 && parameters[0].text === "self" ? 1 : 0;
  ...
  for (let i = startParamIndex; i < parameters.length; i++) {
    ...
    switch (parameterNode.type) {
      case "identifier":
        snippet +=
          "@param " + parameterNode.text + ": $" + cursorCounter + "\n";
        cursorCounter++;
        break;
...
function getReSTDocstringSnippet(
  parameters: SyntaxNode[],
  currentNode: SyntaxNode
): string {
  let cursorCounter = 1;
